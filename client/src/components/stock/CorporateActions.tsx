          ...
          <tbody className="divide-y divide-white/6">
            {(data?.actions ?? []).map((action, index) => (
              <tr key={`${action.type}-${action.effectiveAt ?? index}`}>
                <td className="py-3 font-medium text-white">{action.type}</td>
                <td className="py-3 text-slate-300">
                  {action.amount ? formatConvertedMonetaryValue(action.amount) : '—'}
                </td>
                <td className="py-3 text-slate-300">{action.ratio || '—'}</td>
                <td className="py-3 text-slate-300">{action.effectiveAt || '—'}</td>
                <td className="py-3 text-slate-300">{action.announcedAt || '—'}</td>
                <td className="py-3 text-slate-300">{action.recordDate || '—'}</td>
                <td className="max-w-xs py-3 text-slate-500">{action.description || '—'}</td>
              </tr>
            ))}
          </tbody>
        </table>
